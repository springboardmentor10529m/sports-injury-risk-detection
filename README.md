...
### Project Objectives
1. **Centralized Profile Management**: Provide a secure platform for athletes to maintain physical baselines (sport, position, age, height, weight).
2. **Secure Movement Video Vault**: Store raw movement video binaries (MP4, MOV, AVI, WebM) directly in PostgreSQL for data integrity and AI model pipeline access.
3. **Profile-Gated Assessment Workflow**: Enforce profile completeness checks to ensure physical parameters are recorded prior to video submissions.
4. **Role-Based Collaboration**: Enable seamless interaction between Athletes, Coaches, Physiotherapists, Sports Scientists, and Administrators.
5. **Extensible AI Architecture**: Prepare database schemas, REST APIs, and UI pipelines for seamless MediaPipe pose estimation, ML risk scoring, and automated recommendation generation.
...
Detailed architectural specifications, data flows, and database ERDs are available in the [`docs/`](file:///c:/Users/Welcome/sports-injury-risk-detection/docs) folder:
* 📄 [docs/workflow.md](file:///c:/Users/Welcome/sports-injury-risk-detection/docs/workflow.md): Complete end-to-end user workflow and planned AI pipeline.
* 📄 [docs/architecture.md](file:///c:/Users/Welcome/sports-injury-risk-detection/docs/architecture.md): Technical architecture, JWT security sequence, and video storage pipeline.
* 📄 [docs/database.md](file:///c:/Users/Welcome/sports-injury-risk-detection/docs/database.md): Database schema descriptions, PostgreSQL ERD, and migration history.
* 📄 [docs/wireframes/README.md](file:///c:/Users/Welcome/sports-injury-risk-detection/docs/wireframes/README.md): Low-fidelity UI/UX wireframes for all pages.
...
```
Interactive documentation: [http://127.0.0.1:8000/api/v1/docs](http://127.0.0.1:8000/api/v1/docs)
...
npm run dev
```
Access application UI: [http://localhost:5173](http://localhost:5173)
