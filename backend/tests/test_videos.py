...
async def test_successful_video_upload(client: AsyncClient, test_user: User, athlete_profile_1: AthleteProfile):
    ...
    files = {
        "file": ("jump_test.mp4", VALID_MP4_BYTES, "video/mp4")
    }
    data = {
        "sport_type": "Drop Jump Test"
    }
...
async def test_athlete_can_only_upload_for_self(
    ...
    files = {
        "file": ("drill.mp4", VALID_MP4_BYTES, "video/mp4")
    }
    data = {
        "athlete_id": str(athlete_profile_2.id)
    }
...
@pytest.mark.asyncio
async def test_coach_can_upload_for_athlete(
    client: AsyncClient,
    coach_user: User,
    athlete_profile_1: AthleteProfile
):
    ...
    files = {
        "file": ("sprint.mp4", VALID_MP4_BYTES, "video/mp4")
    }
    data = {
        "athlete_id": str(athlete_profile_1.id),
        "sport_type": "Max Sprint"
    }
...
async def test_upload_invalid_extension(client: AsyncClient, test_user: User, athlete_profile_1: AthleteProfile):
    ...
    files = {
        "file": ("malicious.txt", b"not a video", "text/plain")
    }
    response = await client.post(
        "/api/v1/videos/upload",
        files=files,
...
async def test_upload_invalid_mime(client: AsyncClient, test_user: User, athlete_profile_1: AthleteProfile):
    ...
    files = {
        "file": ("fake.mp4", VALID_MP4_BYTES, "application/pdf")
    }
    response = await client.post(
        "/api/v1/videos/upload",
        files=files,
...
async def test_upload_corrupted_file(client: AsyncClient, test_user: User, athlete_profile_1: AthleteProfile):
    ...
    files = {
        "file": ("corrupt.mp4", b"<html><body>Hello Fake Video</body></html>", "video/mp4")
    }
    response = await client.post(
        "/api/v1/videos/upload",
        files=files,
...
async def test_upload_oversized_file(
    ...
    files = {
        "file": ("large.mp4", VALID_MP4_BYTES * 100, "video/mp4")
    }
    response = await client.post(
        "/api/v1/videos/upload",
        files=files,
...
async def test_database_record_creation(
    ...
    files = {
        "file": ("cut_drill.mp4", VALID_MP4_BYTES, "video/mp4")
    }
    response = await client.post(
        "/api/v1/videos/upload",
        files=files,
...
async def test_path_traversal_sanitization(
    ...
    files = {
        "file": ("../../../../etc/evil.mp4", VALID_MP4_BYTES, "video/mp4")
    }
    response = await client.post(
        "/api/v1/videos/upload",
        files=files,
