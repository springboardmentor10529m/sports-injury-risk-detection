...
async def test_add_injury_record(client: AsyncClient, test_user, physio_user, db_session):
    ...
    profile = AthleteProfile(id=uuid4(), user_id=test_user.id, sport="Soccer", position="Forward")
...
async def test_log_training(client: AsyncClient, test_user, db_session):
    ...
    profile = AthleteProfile(id=uuid4(), user_id=test_user.id, sport="Soccer")
    db_session.add(profile)
    await db_session.commit()
    await db_session.refresh(profile)
...
async def test_full_flow(client: AsyncClient):
    ...
    login = await client.post(
        "/api/v1/auth/login",
        data={"username": "flowtest@test.com", "password": "testpass123"},
    )
    ...
    profile = await client.post(
        "/api/v1/athletes/profile",
        json={"user_id": user_id, "sport": "Basketball", "position": "Guard"},
        headers=headers,
    )
