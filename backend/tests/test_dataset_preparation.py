...
def test_dataset_validator_missing_labels_and_corrupt(tmp_path: Path):
    ...
    report = validator.validate_manifest(manifest, check_files_exist=True, check_video_integrity=True, allow_unlabeled=False)
...
def test_dataset_validator_duplicate_detection(tmp_path: Path):
    ...
    samples = [
        DatasetSample(video_id="V1", video_path=str(vid_file), injury_label=0, source_dataset="Dup_Test"),
        DatasetSample(video_id="V2", video_path=str(dup_file), injury_label=0, source_dataset="Dup_Test"),
    ]
...
def test_dataset_ingestion_and_feature_pipeline(tmp_path: Path):
    ...
    with open(csv_file, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow([
            "video_id", "athlete_id", "video_path", "sport", "movement_type",
            "injury_label", "injury_type", "laterality", "timestamp_onset_seconds"
        ])
        writer.writerow(["V_01", "ATH_10", str(v1_path), "Soccer", "Drop Jump", "0", "NONE", "BILATERAL", "0.5"])
        writer.writerow(["V_02", "ATH_20", str(v2_path), "Basketball", "Drop Jump", "1", "ACL_TEAR", "LEFT", "0.8"])

    # 1. Ingest Dataset
    ingestor = DatasetIngestor(storage_manager=storage)
    provenance = DatasetProvenance(
        source_name="Integration_Test_Cohort",
        version="1.0.0",
        license="MIT"
    )
