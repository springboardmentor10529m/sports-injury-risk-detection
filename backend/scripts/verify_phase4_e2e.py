...
def create_dynamic_motion_video(num_frames=25) -> bytes:
    ...
    try:
        ...
        for i in range(num_frames):
            ...
            # Head
            cv2.circle(frame, (320 + trunk_tilt_offset, 100 + squat_offset), 25, (255, 255, 255), -1)
            # Torso
            cv2.line(frame, (320 + trunk_tilt_offset, 125 + squat_offset), (320, 260 + squat_offset), (255, 255, 255), 6)
            # Arms
            cv2.line(frame, (320 + trunk_tilt_offset, 160 + squat_offset), (240, 220 + squat_offset), (255, 255, 255), 4)
            cv2.line(frame, (320 + trunk_tilt_offset, 160 + squat_offset), (400, 210 + squat_offset), (255, 255, 255), 4)
            # Left Leg (pronounced flexion)
            cv2.line(frame, (320, 260 + squat_offset), (250, 360 + int(squat_offset * 1.4)), (255, 255, 255), 4)
            cv2.line(frame, (250, 360 + int(squat_offset * 1.4)), (250, 450), (255, 255, 255), 4)
            # Right Leg (stiff landing -> creates bilateral asymmetry)
            cv2.line(frame, (320, 260 + squat_offset), (380, 340 + int(squat_offset * 0.8)), (255, 255, 255), 4)
            cv2.line(frame, (380, 340 + int(squat_offset * 0.8)), (380, 450), (255, 255, 255), 4)
...
async def run_phase4_e2e_verification():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        for key, dev in list(anom_data["metric_deviations"].items())[:4]:
            ...
            print(f"       - Observed: {dev['observed']}{dev['unit']} | Baseline Mean: {dev['baseline_mean']}{dev['unit']} ± {dev['baseline_std']}{dev['unit']}")
            print(f"       - Z-Score: {dev['z_score']} | Percent Dev: {dev['percent_deviation']}% | Range Dev: {dev['range_deviation']}")
