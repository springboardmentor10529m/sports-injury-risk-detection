...
from app.ml.datasets.schema import DataModality, DatasetManifest
...
class ValidationReport(BaseModel):
    ...
    duplicate_records: list[dict[str, Any]] = Field(
        default_factory=list, description="Detected duplicate file checksums or IDs"
    )
    corrupt_files: list[dict[str, Any]] = Field(
        default_factory=list, description="Unreadable or corrupt video/data files"
    )
    ...
    class_imbalance_ratio: float | None = Field(default=None, description="Ratio between majority and minority class")
    athlete_distribution: dict[str, int] = Field(
        default_factory=dict, description="Sample counts grouped by athlete_id"
    )
    ...
    def validate_manifest(
        ...
        for sample in manifest.samples:
            ...
            if missing_meta_fields:
                ...
                report.warnings.append(f"Sample {sample.sample_id} missing detailed metadata: {missing_meta_fields}")
                                    ...
                                    report.corrupt_files.append(
                                        {
                                            ...
                                            "file_path": target_path_str,
                                            "reason": (
                                                f"Insufficient frames: {frame_count} "
                                                f"(< 5 frames required for kinematics)."
                                            ),
                                        }
                                    )
                                    sample_errors.append(
                                        f"Too few frames ({frame_count}) for sample {sample.sample_id}"
                                    )
                                    ...
                                    report.corrupt_files.append(
                                        {
                                            ...
                                            "file_path": target_path_str,
                                            "reason": (
                                                f"Invalid video dimensions or framerate: {width}x{height} @ {fps}fps."
                                            ),
                                        }
                                    )
                                    sample_errors.append(f"Invalid video metadata for sample {sample.sample_id}")

            elif sample.modality in [
                DataModality.BIOMECHANICAL_TABLE,
                DataModality.MOTION_CAPTURE,
            ]:
                # Non-file tabular records must have biomechanical data
                if not sample.biomechanical_data and not sample.metadata:
                    report.warnings.append(
                        f"Sample {sample.sample_id} is marked as {sample.modality.value} "
                        f"but has empty biomechanical_data dictionary."
                    )
        ...
        if len(class_counts) > 1:
            ...
            single_cls = list(class_counts.keys())[0]
            single_cnt = list(class_counts.values())[0]
            report.warnings.append(f"Single class dataset: only class '{single_cls}' present ({single_cnt} samples).")
