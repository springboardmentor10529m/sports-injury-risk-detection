...
from app.schemas.analysis import AnomalyAssessmentResponse, KinematicsResponse
...
async def get_kinematics_analysis(
    ...
    result = await pipeline.get_kinematics(video_id=video_id, current_user=current_user, db=db)
...
async def get_anomalies_analysis(
    ...
    """
    pipeline = get_pipeline_service()
    result = await pipeline.get_anomalies(video_id=video_id, current_user=current_user, db=db)
