...
class AthleteService:
    ...
    async def create_profile(self, profile_in: AthleteProfileCreate) -> AthleteProfile:
        ...
        # Check user exists
        result = await self.db.execute(select(User).where(User.id == profile_in.user_id))
        user = result.scalar_one_or_none()
        if not user:
            raise NotFoundError(f"User '{profile_in.user_id}' not found")

        # Check no existing profile
        existing = await self.db.execute(select(AthleteProfile).where(AthleteProfile.user_id == profile_in.user_id))
    ...
    async def get_profile(self, athlete_id: UUID) -> AthleteProfile:
        ...
        result = await self.db.execute(select(AthleteProfile).where(AthleteProfile.id == athlete_id))
        profile = result.scalar_one_or_none()
        if not profile:
            raise NotFoundError(f"Athlete profile '{athlete_id}' not found")
        return profile

    async def get_profile_by_user_id(self, user_id: UUID) -> AthleteProfile | None:
        """Get athlete profile by user ID."""
        result = await self.db.execute(select(AthleteProfile).where(AthleteProfile.user_id == user_id))
        return result.scalar_one_or_none()

    async def update_profile(self, athlete_id: UUID, profile_update: AthleteProfileUpdate) -> AthleteProfile:
    ...
    async def add_injury_record(self, athlete_id: UUID, injury_in: InjuryHistoryCreate) -> InjuryHistory:
        """Add an injury record to an athlete's history."""
        await self.get_profile(athlete_id)  # Verify athlete exists
        record = InjuryHistory(athlete_id=athlete_id, **injury_in.model_dump())
        self.db.add(record)
        await self.db.commit()
    ...
    async def log_training(self, athlete_id: UUID, load_in: TrainingLoadCreate) -> TrainingLoad:
        """Log a training session."""
        await self.get_profile(athlete_id)  # Verify athlete exists
        load = TrainingLoad(athlete_id=athlete_id, **load_in.model_dump())
        self.db.add(load)
        await self.db.commit()
        await self.db.refresh(load)
        return load

    async def get_training_load(self, athlete_id: UUID, limit: int = 50) -> list[TrainingLoad]:
